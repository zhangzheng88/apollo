...
public class NamespaceBranchController {
  ...
  public void deleteBranch(@PathVariable String appId,
    ...
    namespaceBranchService
        .deleteBranch(appId, Env.valueOf(env), clusterName, namespaceName, branchName);
  ...
  public ReleaseDTO merge(@PathVariable String appId, @PathVariable String env,
    ...
    if (model.isEmergencyPublish() && !portalConfig
        .isEmergencyPublishAllowed(Env.fromString(env))) {
      throw new BadRequestException(
          String.format("Env: %s is not supported emergency publish now", env));
    }

    ReleaseDTO createdRelease = namespaceBranchService
        .merge(appId, Env.valueOf(env), clusterName, namespaceName, branchName,
  ...
  public void updateBranchRules(@PathVariable String appId, @PathVariable String env,
    ...
    namespaceBranchService
        .updateBranchGrayRules(appId, Env.valueOf(env), clusterName, namespaceName, branchName,
            rules);
