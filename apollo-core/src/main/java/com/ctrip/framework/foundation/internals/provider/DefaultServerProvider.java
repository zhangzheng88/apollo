...
//    }
    ...
    if (!Utils.isBlank(m_env)) {
      m_env = m_env.trim();
      logger.info("Environment is set to [{}] by property 'apollo.env' in container.properties.",
          m_env);
      return;
    }
    ...
    if (!Utils.isBlank(m_env)) {
      ...
      logger
          .info("Environment is set to [{}] by OS env variable 'APPLICATION_STANDARD_ENV'.", m_env);
      return;
    }
    ...
    m_env = "daily";
    logger.warn(
        "Environment is set to daily. Because it is not available in either (1) JVM system property 'apollo.env', (2) OS env variable 'APPLICATION_STANDARD_ENV' nor (3) property 'apollo.env' from the properties InputStream.");
  }
...
//    }
    ...
    if (!Utils.isBlank(m_dc)) {
      m_dc = m_dc.trim();
      logger
          .info("Data Center is set to [{}] by property 'apollo.cluster' in container.properties.",
              m_dc);
      return;
    }
//     2. Try to get idc from OS environment variable
